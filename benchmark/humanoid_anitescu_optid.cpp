...
auto printInertiaMCoMI = [](const std::string &tag, const pinocchio::Inertia &I)
{
  const auto pi = I.toDynamicParameters(); // 10x1

  const double m = I.mass();
  const Eigen::Vector3d h = pi.segment<3>(1); // [hx,hy,hz] = m*com
  const Eigen::Vector3d com = h / m;

  Eigen::Matrix3d Ic = I.inertia();
  std::cout << "========== " << tag << " ==========\n";
  std::cout << "m = " << m << "\n";
  std::cout << "h = [m*com] = " << h.transpose() << "\n";
  std::cout << "com (in this inertia frame) = " << com.transpose() << "\n";
  std::cout << "Inertia matrix about frame origin (same frame as com):\n"
            << Ic << "\n\n";
};
...
int main(int argc, char *argv[])
{
  ...
  std::vector<std::string> files = {
      "data_g1_anitescu/f_log.csv",
      "data_g1_anitescu/mode_log.csv",
      "data_g1_anitescu/us_log.csv",
      "data_g1_anitescu/xs_log.csv",
      "data_g1_anitescu/us_results_fddp.csv",
      "data_g1_anitescu/xs_results_fddp.csv",
      "data_g1_anitescu/fs_log.csv",
      "data_g1_anitescu/hls_log.csv",
      "data_g1_anitescu/hrs_log.csv",
  };
  ...
  // ------------------------------------------------------------------
  char *path = "/home/jkang/third_party/PRIME/data_g1_anitescu/est_params_id.xml";
  ...
  // Eigen::MatrixXd u0_init = csvutil::readCSVtoEigen("data_g1_anitescu/high_kappa_nopayload/u0_results_fddp.csv");
  // pi_log = pi_log + u0_init.row(0).transpose();
  ...
  Eigen::MatrixXd qs = csvutil::readCSVtoEigen("/home/jkang/third_party/PRIME/data_g1_anitescu/mocap_hand/p_sim.csv");
  Eigen::MatrixXd vs = csvutil::readCSVtoEigen("/home/jkang/third_party/PRIME/data_g1_anitescu/mocap_hand/v_sim.csv");
  Eigen::MatrixXd us = csvutil::readCSVtoEigen("/home/jkang/third_party/PRIME/data_g1_anitescu/mocap_hand/tau_sim.csv");
  // Eigen::MatrixXd fs = csvutil::readCSVtoEigen("/home/jkang/third_party/PRIME/data_g1_anitescu/mocap_dance/f_sim.csv");
  Eigen::MatrixXd hls = csvutil::readCSVtoEigen("/home/jkang/third_party/PRIME/data_g1_anitescu/mocap_hand/left_sim.csv");
  Eigen::MatrixXd hrs = csvutil::readCSVtoEigen("/home/jkang/third_party/PRIME/data_g1_anitescu/mocap_hand/right_sim.csv");
  ...
  for (std::size_t i = 0; i < horizon; i += down_sample)
  {
    ...
    state_init.push_back(xs_i);
    csvutil::logVectorToCSV(xs_i, "data_g1_anitescu/xs_log.csv");
  }
  ...
  for (std::size_t i = 0; i + down_sample < horizon; i += down_sample)
  {
    ...
    ctrl_init.push_back(ctrl_full_i);

    Eigen::VectorXd hl_i = hls.row(i + start_idx).transpose();
    Eigen::VectorXd hr_i = hrs.row(i + start_idx).transpose();
    // hl_i[3] = hl_i[3] - 0.120737;
    // hr_i[3] = hr_i[3] - 0.120737;
    csvutil::logVectorToCSV(hl_i, "data_g1_anitescu/hls_log.csv");
    csvutil::logVectorToCSV(hr_i, "data_g1_anitescu/hrs_log.csv");
    csvutil::logVectorToCSV(ctrl_full_i, "data_g1_anitescu/us_log.csv");
  }
  ...
  // Print (m, com, inertia matrix)
  printInertiaMCoMI("xs_sol (I1)", I1);
  printInertiaMCoMI("pi_log (I2)", I2);
  ...
  csvutil::saveEigenToCSV("data_g1_anitescu/xs_results_fddp.csv", xs_results);
  csvutil::saveEigenToCSV("data_g1_anitescu/us_results_fddp.csv", us_results);

  const int nu0 = us_sol[0].size();
  Eigen::MatrixXd u0_mat(1, nu0);
  u0_mat.row(0) = us_sol[0].transpose();
  csvutil::saveEigenToCSV("data_g1_anitescu/u0_results_fddp.csv", u0_mat);
  ...
  csvutil::saveEigenToCSV("data_g1_anitescu/xs_rollout.csv", xs_roll);
}
