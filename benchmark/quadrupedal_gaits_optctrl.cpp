...
int main(int argc, char *argv[])
{
  ...
  crocoddyl::SimpleQuadrupedGaitProblem gait(model, "lf_foot", "rf_foot",
                                             "lh_foot", "rh_foot");
  std::cout << model.effortLimit << std::endl;
  ...
  std::cout << "Init u size: " << us.size() << std::endl;
  xs.push_back(x0);
  ...
  // Eigen::MatrixXd F_u_numerical = Eigen::MatrixXd::Zero(model.nv * 2, model.nv - 6);
  // for (size_t i = 0; i < model.nv - 6; ++i)
  // {
  //   Eigen::VectorXd x_next_add = Eigen::VectorXd::Zero(model.nv + model.nq);
  //   Eigen::VectorXd u_add = Eigen::VectorXd::Zero(model.nv - 6);
  //   u_add(i) = epsilon;
  //   Eigen::VectorXd u_test = us[0] + u_add;
  //   problem->running_models_[0]->calc(data, xs[0], u_test);
  //   x_next_add = data->xnext;
  //   Eigen::VectorXd v_diff = x_next_add.tail(model.nv) - x_next.tail(model.nv);

  //   Eigen::VectorXd x_diff_tangential = Eigen::VectorXd::Zero(model.nv);
  //   pinocchio::difference(model, x_next.head(model.nq), x_next_add.head(model.nq), x_diff_tangential);
  //   F_u_numerical.block(0, i, model.nv, 1) = x_diff_tangential / epsilon;
  //   F_u_numerical.block(model.nv, i, model.nv, 1) = v_diff / epsilon;
  // }
