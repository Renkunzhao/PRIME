...
def resolve_results_dir(experiment_dir):
    if len(sys.argv) > 2:
        raise SystemExit("Usage: meshcat_go2.py [results_dir]")
    if len(sys.argv) == 2:
...
def require_file(path):
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "Run the Go2 experiment first or pass a results directory, e.g.\n"
            "  python3 experiments/Go2_real_belly_plate_4.6kg/visualizer/meshcat_go2.py "
            "experiments/Go2_real_belly_plate_4.6kg/results/stage_01_kappa_200"
        )
    return path
