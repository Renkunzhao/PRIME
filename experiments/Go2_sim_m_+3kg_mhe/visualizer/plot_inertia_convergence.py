...
def plot_convergence(data, output_path, x_key):
    ...
    fig.suptitle("Go2 MHE Inertia Convergence", fontsize=14)
...
def plot_hessian_diagonal(data, output_path, x_key, source):
    ...
    if not diag:
        raise ValueError(
            ...
            "Rerun go2_sim_m_p3kg_mhe with the updated logger."
        )
    ...
    fig.suptitle(f"Go2 MHE {title_source}", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot mass, inertia diagonal, and COM convergence from Go2 MHE summary output."
    )
    parser.add_argument(
        "--summary",
        type=Path,
