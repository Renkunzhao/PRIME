...
def resolve_window_dir(experiment_dir, args):
    ...
    if not args.results_root.is_absolute():
        results_root = (experiment_dir / args.results_root).resolve()

    if args.window_dir is not None:
        return args.window_dir.expanduser().resolve()
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Visualize one Go2 shifted-COM MHE window: optimized trajectory vs raw/reference state log."
    )
    parser.add_argument(
        "window_dir",
        nargs="?",
...
def require_file(path):
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "  python3 experiments/Go2_sim_cz_-0.1m_mhe/visualizer/meshcat_go2_window.py "
            "experiments/Go2_sim_cz_-0.1m_mhe/results/window_000_start_0200"
        )
    return path
...
def main():
    ...
    baseline_dir = experiment_dir.parent / experiment_dir.name.removesuffix("_mhe")
    descriptions_dir = baseline_dir / "descriptions"
    ...
    print("Using baseline descriptions:", descriptions_dir)
