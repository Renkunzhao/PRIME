...
int main(int argc, char* argv[]) {
  try {
    if (argc != 2) {
      std::cerr << "Usage: go2_sim_kunzhao_mhe <config.xml>\n";
      return 2;
    }
    ...
    if (cfg.solver.dry_run) {
      std::cout << "Parsed Go2 Kunzhao MHE contact-ID XML successfully.\n";
    ...
    std::cout
        << "Go2 Kunzhao MHE carries parameter information from the dropped horizon "
    ...
    std::cerr << "go2_sim_kunzhao_mhe: " << e.what() << "\n";
    return 1;
  }
  return 0;
}
