...
def plot_convergence(data, output_path, x_key, gt=None):
    ...
    fig.suptitle("Go2 Kunzhao Long MHE Inertia Convergence", fontsize=14)
...
def plot_hessian_diagonal(data, output_path, x_key, source):
    ...
    if not diag:
        raise ValueError(
            ...
            "Rerun go2_sim_kunzhao_long_mhe with the updated logger."
        )
    ...
    fig.suptitle(f"Go2 Kunzhao Long MHE {title_source}", fontsize=14)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot mass, inertia diagonal, and COM convergence from Go2 Kunzhao Long MHE summary output."
    )
    parser.add_argument(
        "--summary",
        type=Path,
