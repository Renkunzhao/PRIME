...
def main():
    ...
    parser.add_argument(
        "--all-data",
        action="store_true",
        help="play from raw row 0 through the end; disables the max-frame cap",
    )
    ...
    start_idx = 0 if args.all_data and args.start_idx is None else (
        cfg["start_idx"] if args.start_idx is None else args.start_idx
    )
    stride = cfg["down_sample"] if args.stride is None else args.stride
    max_frames = None if args.all_data else args.max_frames
    ...
    qs, qs_raw_order, times = load_q_data(
        cfg, model, start_idx, stride, max_frames, use_joint_order
    )

    print("Using q data:", cfg["q_csv"])
    print("Using URDF:", cfg["urdf"])
    end_idx = start_idx + stride * (len(qs) - 1) if len(qs) else start_idx
    print(
        f"Loaded q frames={len(qs)} start_idx={start_idx} "
        f"end_idx={end_idx} stride={stride}"
    )
