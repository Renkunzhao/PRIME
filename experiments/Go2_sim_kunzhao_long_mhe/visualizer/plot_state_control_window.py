#!/usr/bin/env python3
"""Plot optimized state/control against logs for one MHE result window."""
...
def load_config(path):
    ...
    if paths is None or robot is None or outputs is None:
        raise ValueError("Config is missing paths, robot, or outputs block.")
    ...
    return {
        ...
        "results": Path(variables["results"]),
...
def resolve_cli_path(experiment_dir, path):
...
def resolve_window_dir(experiment_dir, cfg, args):
    results_root = (
        resolve_cli_path(experiment_dir, args.results_root)
        if args.results_root is not None
        else cfg["results"].resolve()
    )

    if args.window_dir is not None:
        return resolve_cli_path(experiment_dir, args.window_dir)
...
def main():
    ...
    default_config = experiment_dir / "config" / "Go2_sim_kunzhao_long_mhe.xml"
    parser = argparse.ArgumentParser(
        description="Compare optimized q/v/u against logs for one Go2 Kunzhao Long MHE window."
    )
    parser.add_argument(
        "window_dir",
        nargs="?",
        type=Path,
        help="specific window result directory, e.g. results/window_012_start_10480",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="output directory; default is <window_dir>/state_control_comparison",
    )
    ...
    results_dir = resolve_window_dir(experiment_dir, cfg, args)
    output_dir = (
        resolve_cli_path(experiment_dir, args.output_dir)
        if args.output_dir is not None
        else results_dir / "state_control_comparison"
    )
    ...
    print(f"Using window: {results_dir}")
    print(f"Writing plots to: {output_dir}")
    pdf_path = output_dir / "state_control_comparison.pdf"
    with PdfPages(pdf_path) as pdf:
        plot_group(
            pdf, output_dir, "q", "Generalized Position q", state_x,
            q_log, q_opt, q_labels, args.per_page
        )
        plot_group(
            pdf, output_dir, "v", "Generalized Velocity v", state_x,
            v_log, v_opt, v_labels, args.per_page
        )
        plot_group(
            pdf, output_dir, "u", "Control u", ctrl_x,
